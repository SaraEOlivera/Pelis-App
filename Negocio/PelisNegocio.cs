    ...
    public class PelisNegocio
    {
        ...
        public List<Pelicula> listar()
        {
            ...
            try
            {
                ...
                comando.CommandText = "With PlataformasUnicas as (Select PP.IdPelicula, STRING_AGG(PL.Nombre, ', ') as Plataformas From [Peliculas.Plataformas] PP\r\n    inner join Plataformas PL ON PP.IdPlataforma = PL.Id group by PP.IdPelicula) Select P.Titulo, P.Bio, STRING_AGG(C.Descripcion, ', ') as 'Genero', Cl.Descripcion as 'Clasificacion', MIN(M.MediaURL) as 'Imagen', P.FechaEstreno,  PU.Plataformas From Peliculas P inner join [Peliculas.Categorias] PC on P.Id = PC.IdPelicula inner join Categorias C on PC.IdCategoria = C.Id inner join [Peliculas.Clasificaciones] PCl on P.Id = PCl.IdPelicula inner join Clasificaciones Cl on PCl.IdClasificacion = Cl.Id inner join Media M on P.Id = M.IdPelicula inner join PlataformasUnicas PU on P.Id = PU.IdPelicula group by P.Titulo, P.Bio, P.FechaEstreno, Cl.Descripcion, PU.Plataformas;";
